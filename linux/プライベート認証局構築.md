...
## 6. Apacheのクライアント証明書認証設定
    ...
    <VirtualHost *:443>
        ...
        SSLCertificateFile /etc/pki/tls/server/server.crt
        SSLCertificateKeyFile /etc/pki/tls/server/server.key
    ...
    ```
## 7. 動作確認を行う
    ローカルPCにて、[client.p12]ファイルを開いてクライアント証明書をインストールし、
    ブラウザからWebサーバーにアクセスする。
    警告メッセージと、クライアント証明書の選択画面が表示されることを確認する
