...
## 実際のCI/CD構築
  ...
  53. サービスから[ECS]にアクセスし、サイドメニューの「クラスター」より「サービス」を選択する
  54. [タスク]タブをクリックして、設定から[パブリックIP]に表示されているIPでブラウザ検索する
  55. アップロードしたページが表示されることを確認する

## 課題
  1. 秘匿情報の管理（環境変数、シークレットキー・ID）
  2. CodeCommitへのPushやGitlabのマスターブランチへのmergeリクエスト実行権限の制御
  3. 最小構成の見直し（意見をもらう）
